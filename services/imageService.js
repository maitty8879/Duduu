    ...
    try {
        ...
        try {
            ...
            if (!result || !result.imageUrl) {
                throw new Error('未从Coze工作流响应中提取到图片URL');
            }
        } catch (apiError) {
        ...
        // 从结果中提取图片URL
        if (!result || !result.imageUrl) {
            console.warn('未从响应中提取到图片URL，使用模拟服务');
            // 没有找到图片URL，使用模拟服务
            result = await mockCozeService.runWorkflow(prompt, useStream);
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            cozeService.runWorkflow(prompt, true).catch(err => {
                ...
                // 如果还没有完成，并且没有使用过后备方案
                if (!isCompleted && !fallbackUsed) {
                    fallbackUsed = true;
                    console.log('尝试使用模拟服务作为后备...');
                    ...
                    // 启动模拟工作流
                    mockCozeService.runWorkflow(prompt, true).catch(mockErr => {
                        console.error('模拟服务也失败:', mockErr);
                        handleServiceFailure(mockErr);
                    });
                } else if (!isCompleted) {
                    handleServiceFailure(err);
                }
            });
