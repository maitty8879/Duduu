...
app.post(['/generate-image', '/api/coze/workflow'], async (req, res) => {
  try {
    ...
    try {
      ...
      // 尝试不同的API基础URL
      const cozeApiBaseOptions = [
        'https://api.coze.cn',
        'https://api.coze.com',
        'https://api-cn.coze.cn',
        'https://openapi.coze.cn'
      ];
      ...
      // 尝试所有可能的API基础URL
      for (const baseUrl of cozeApiBaseOptions) {
        try {
          console.log(`尝试调用 Coze API: ${baseUrl}/open-api/v2/workflow-runs`);
          
          response = await axios.post(
            `${baseUrl}/open-api/v2/workflow-runs`,
            { 
            ...
            {
              headers: {
                'Authorization': `Pat ${cozeApiToken}`,
          ...
          console.log(`${baseUrl} 响应:`, response.data);
          
          // 如果没有抛出错误，说明这个URL是有效的
          success = true;
          apiBase = baseUrl;
          break;
        } catch (err) {
          console.log(`${baseUrl} 调用失败:`, err.message);
          // 继续尝试下一个URL
        }
      }
      ...
      if (!success) {
        // 所有URL都失败了
        return res.status(500).json({ error: '无法连接到Coze API，请检查API密钥和网络连接' });
      }
      ...
      const pollResult = async () => {
        try {
          console.log(`检查工作流状态: ${apiBase}/open-api/v2/workflow-runs/${runId}`);
          const statusResponse = await axios.get(
            `${apiBase}/open-api/v2/workflow-runs/${runId}`,
            {
              headers: {
                'Authorization': `Pat ${cozeApiToken}`,
                'Content-Type': 'application/json'
...
app.get('/api/coze/workflow/status', async (req, res) => {
  try {
    ...
    for (const baseUrl of cozeApiBaseOptions) {
      try {
        console.log(`尝试检查工作流状态: ${baseUrl}/open-api/v2/workflow-runs/${runId}`);
        
        statusResponse = await axios.get(
          `${baseUrl}/open-api/v2/workflow-runs/${runId}`,
          {
            headers: {
              'Authorization': `Pat ${cozeApiToken}`,
              'Content-Type': 'application/json'
        ...
        console.log(`${baseUrl} 状态响应:`, statusResponse.data);
        
        // 如果没有抛出错误，说明这个URL是有效的
        success = true;
        break;
      } catch (err) {
        console.log(`${baseUrl} 状态检查失败:`, err.message);
        // 继续尝试下一个URL
      }
    }
